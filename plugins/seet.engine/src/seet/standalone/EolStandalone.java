...
public class EolStandalone extends EpsilonStandalone{
	...
	public EolStandalone(FrontPage frontPage)
	{
		model = FrontPage.inputModel;
		inputMetaModel = frontPage.getText2();
		outputMetamodel = frontPage.getText3();
	}
	...
	public List<IModel> getModels() throws Exception {
		...
		URI emfURI1 = URI.createFileURI(outputMetamodel);
		registerMetamodel(outputMetamodel);
		...
		Resource resource2 = resourceSet1.getResource(emfURI1, true);
		EPackage outputPackage = (EPackage)resource2.getContents().get(0);		

		models.add(createEmfMetaModelByURI("IN_MM", "IN_MM", inputPackage.getNsURI()));
		models.add(createEmfMetaModelByURI("OUT_MM", "OUT_MM", outputPackage.getNsURI()));
		...
		models.add(createEmfModelByURI("CFG", path + "CFGModel.model", 
				"CFG", true, true));
		models.add(createEmfModelByURI("Ecore", path + "SMF.model", 
				"http://www.eclipse.org/emf/2002/Ecore", false, true));
		models.add(createEmfModelByURI("ATLOutput", path + "ATLExpression.model",
				"http://www.eclipse.org/gmt/2005/ATL", false, true));
		return models;
	}
